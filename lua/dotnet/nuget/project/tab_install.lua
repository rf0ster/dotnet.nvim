...
--- @param on_exit function Called after the uninstall command finishes.
local function uninstall_keymap(val, proj_file, output_bufnr, on_exit)
    confirm.open({
        ...
        on_confirm = function()
            cli.new(output_bufnr, nil, on_exit):remove_package(proj_file, val.value.id)
        end
    })
end
...
function M.open(proj_file)
    ...
    -- after a mutation (e.g., uninstall) completes.
    ...
    picker = require "dotnet.nuget.picker":new({
        ...
        keymaps = {
            {
                key = "<leader>u",
                callback = function(val) uninstall_keymap(val, proj_file, output_bufnr, refresh_pkgs) end
            }
        }
    })
