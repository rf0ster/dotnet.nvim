...
local function map_to_results_async(search_term, pkgs, state, callback)
...
function M.open(proj_file)
    ...
    local picker
    -- Re-reads the installed packages and refreshes the picker
    -- after an update completes so updated packages drop off the list.
    local refresh_pkgs = vim.schedule_wrap(function()
        pkgs = require "dotnet.manager".get_nuget_pkgs(proj_file) or {}
        if picker then
    ...
    picker = require "dotnet.nuget.picker":new({
        ...
        row = d.picker.row,
        col = d.picker.col,
        map_to_results_async = function(search_term, callback) map_to_results_async(search_term, pkgs, state, callback) end,
        on_result_selected = function(val) on_result_selected(val, view_bufnr, view_win) end,
        keymaps = {
            {
                key = "<leader>u",
                callback = function(val)
                    cli.new(output_bufnr, output_win, refresh_pkgs)
                        :add_package(proj_file, val.value.id, val.value.latest_version)
                end
            },
            {
                key = "<leader>a",
                callback = function()
                    ...
                    confirm.open({
                        ...
                        prompt = { "Update " .. #outdated .. " package(s) to latest?" },
                        on_confirm = function()
                            -- Chain the updates into one shell command so they
                            -- run sequentially and stream into the output pane.
                            local cmds = {}
                            for _, entry in ipairs(outdated) do
                                table.insert(cmds, "dotnet add " .. proj_file
                                    .. " package " .. entry.value.id
